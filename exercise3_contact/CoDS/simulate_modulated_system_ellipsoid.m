function [DDX,DX,X,DX_G,Time]=simulate_modulated_system_ellipsoid(Center,Radiusx,Radiusy,Target,A,Option)
...
for j=1:size(X_initial,2)
    j
    [DDX{j},DX{j},X{j},DX_G{j},Time{j}] = simulate(X_initial(:,j),A,Target,Center,Radiusx,Radiusy,Option);
end

end

function [DDX,DX,X,DX_G,Time]=simulate(X_initial,A,X_target,Center,Radiusx,Radiusy,option)
...
while ((counter<sizeT))
    ...
    Gamma1=((Center(1)-X(1,counter))^2)/((Radiusx+option.rho)^2)+((Center(2)-X(2,counter))^2)/((Radiusy+option.rho)^2);
    Gamma=((Center(1)-X(1,counter))^2)/((Radiusx)^2)+((Center(2)-X(2,counter))^2)/((Radiusy)^2)-1;
...
function [M,Lambda,CONTACT] = Modulation(Gamma,Gamma1,Radiusx,Radiusy,f_x,DX,delta_dx,rho,q1,q2,Q,Qinv,Contact)
DX_G=q1'*DX;
lambda=zeros(4,1);
epsilon=5;
omega=100;
