...
Data_right = Data([1:3 7:9],:);
att_right = att(1:3,:);
[h_data, h_att, h_vel] = plot_reference_trajectories_DS(Data_right, att_right, vel_samples, vel_size);


Data_left = Data([4:6 10:12],:);
att_left = att(4:end,:);
[h_data, h_att, h_vel] = plot_reference_trajectories_DS(Data_left, att_left, vel_samples, vel_size);
...
[h_gmm]  = visualizeEstimatedGMM(Xi_ref(1:3,:),  ds_gmm.Priors, ds_gmm.Mu(1:3,:), ds_gmm.Sigma(1:3,1:3,:), est_labels, est_options);

% Visualize Estimated Parameters
[h_gmm]  = visualizeEstimatedGMM(Xi_ref(4:end,:),  ds_gmm.Priors, ds_gmm.Mu(4:end,:), ds_gmm.Sigma(4:end,4:end,:), est_labels, est_options);
