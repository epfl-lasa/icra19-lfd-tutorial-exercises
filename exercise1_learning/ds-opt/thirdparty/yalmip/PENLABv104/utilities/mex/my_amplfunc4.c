 ...
 * 
 * The fourth version, my ampl-matlab interface
 * version: 0.6
 * last update: 13/03/09
 *
 ...
 * List of Changes:
 * ----------------
 * added: equalitites -> two inequalitites (EQ2INEQ, transform_flag)
 ...
 *
 *  If transform_flag==1 (see Initialization), equalities are transformed
 *  to two inequalitites so the problem reduces to
 *    min   f(x) 
 *    s.t.  g_i(x)<=0 
 *
 *  USAGE
 *    INITIALIZATION
 *    [[x_guess,v_guess,]output_struct]=amplf('name.nl'[,dense_flag[,transform_flag]])
 ...
 *        sparse structures (dense_flag=0) or dense ones (dense_flag=1). The third
 *        argument is optional as well and specifies whether equality constraints
 *        should be transformed to two inequalitites.
 ...
 *        Note: if there are no inequality and/or equality constraints then g/h are
 *        returned as empty matrices, respectively. But both needs to be specified
 *        as output parameters even if they are empty.
 ...
 *   nrhs=1/2/3, nlhs=1/3, nrhs[0]=string --> initialization
...
#define DEFAULT_TRANSFORM  0   // dtto (e.i., don't transform equalitites unless directly specified by user)
...
static int DENSE=0;
static int EQ2INEQ=0;
//static int MODE=0;

/* DENSE ..... 0..treat problems as sparse, 1..dense
 * EQ2INEQ ... 0..equalitites are unchanged, 1..equalitites are transformed to two inequalitites
...
/* Transformation of i-th constraint lower_bound <= something <= upper_bound to something<=0 
   ...
   somtheing-lower ==0  if (lower==upper)  --> NPERM contains i, BEQUALITY 1
     OR (if EQ2INEQ)
   something-lower ==0 is transformed to something-upper<=0 & lower(==upper)-something<=0 
     --> there will be two entries in NPERM and BEQUALITY, in NPERM i and -(i+1),
         in BEQUALITY two times 1 (but in fact, BEQUALITY is not in use so it doesn't really matter)
	 (N_EQUAL will be zero in this case)
...
static void usage(const char *s)
{
  ...
  if (asl) {
    ...
    mexPrintf("    total equal. c.: %i %s\n",N_EQUAL,EQ2INEQ ? "(equalitites transformed as 2 ineq.)" : "");
    mexPrintf("  \n");
  } else {
  ...
  mexErrMsgTxt("amplf interface usage:\n\n\
    [[x_guess,v_guess,]problem_struct] = amplf('stub'[,dense_flag[,transform_flag]])\n\
       to init the problem\n\
    [f,g,h] = amplf(x,0)                   or\n\
...
/* count number of transformed constraints if there are dim-constraints in the form
   lower[i] <= sth <= upper[i],
   (0=no restriction, 1=equality [if not transformed to two ineq.] or 1 inequality, 2=two inequalities)
...
int count_constr(real *lower, real *upper, int dim, int *noequal) {
  ...
  *noequal=0;
  for (i=0; i<dim; i++) {
    ...
    if (lower[i]==upper[i]) {
      if (EQ2INEQ)
        no++;   // the second constraint as the equality is transformed
      else
        (*noequal)++;
    }
  }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
  ...
  int my_mode, my_dense, my_eq2ineq;
  int i,j,itmp;
  int pos,eqpos;
  double f_val, *t;
  real *xx, *v;
  ...
  /* Initialization: [...,problem_struct]=amplf(name[,density_flag[,transform_flag]]) */
  if ((nrhs==1 || nrhs==2 || nrhs==3) && mxIsChar(prhs[0]) && (nlhs==3 || nlhs==1)) {

    /* find out which mode is used */
    //my_mode = nrhs==2 ? scalarchk(prhs[1]) : 2;
    my_dense = (nrhs==2 || nrhs==3) ? scalarchk(prhs[1]) : DEFAULT_DENSE;
    my_eq2ineq = nrhs==3 ? scalarchk(prhs[2]) : DEFAULT_TRANSFORM;
    if (my_dense!=0 && my_dense!=1)
      usage("dense flag doesn't match");  //error
    if (my_eq2ineq!=0 && my_eq2ineq!=1)
      usage("transform flag doesn't match");  //error
    //MODE=my_mode;
    DENSE=my_dense;
    EQ2INEQ=my_eq2ineq;
	...
	mexPrintf("WARNING: Equality constraints for some variables x_i, treated inefficiently!\nHigh probability of instability!! Do NOT use that!\n");
	  ...
	  if (EQ2INEQ) {   
            // if equalities are transformed to two inequalitites, their order
	    // is rather unchanged
            set_constr(LUv, Uvx, N, BEQUALITY, NPERM);
            set_constr(LUrhs, Urhsx, nc, BEQUALITY+N_BOUNDS, NPERM+N_BOUNDS);
	    memset(bequal_m2,0,N_CONSTR*sizeof(int));
          } else {
            // just to compute the original ('fake') order of equality constraints
