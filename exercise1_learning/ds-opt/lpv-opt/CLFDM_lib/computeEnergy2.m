function [V Vdot] = computeEnergy2(X, Xd, Vxf, beta_eps)
% Syntax:
%
...
if Vxf.SOS
    ...
    [V dV] = GMR_Lyapunov(X, Vxf.Priors, Vxf.Mu, Vxf.P, beta_eps);
end
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [V, Vx] = GMR_Lyapunov(x, Priors, Mu, P, beta_eps)
...
for k = 0:L
    ...
    if k == 0
        ...
        % Original Squared+Asymmetric P's version 
%         V_k(V_k < 0) = 0; % beta
%         V = V + Priors(k+1)*V_k.^2; %alpha.*w.*V_k;
%         Vx = Vx + repmat(2*Priors(k+1)*V_k,d,1).*(P_cur*x_tmp+ P_cur'*x);                
        
        % Alternative Version  V_k+All symmetric matrices
        beta = V_k;
        beta(beta >= beta_eps)  = 1; 
        beta(beta <  beta_eps)  = 0;
        V = V + Priors(k+1)*beta.*V_k;  %alpha.*w.*V_k;        
        Vx = Vx + repmat(beta,d,1).*(P_cur*x_tmp+ P_cur'*x);
