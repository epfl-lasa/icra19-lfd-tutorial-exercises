function [Vxf J] = my_learnEnergy(Vxf0,Data,options)
%
...
%                 ceq(k+1) = scalar*2*L - sum(lambda); % + Vxf.P(:,:,k+1)'                         
                ceq(k+1) = L - sum(lambda); % + Vxf.P(:,:,k+1)'                         
            else
...
%         ceq(k+1) = 2.0 - sum(sum(Vxf.P(:,:,k+1).^2));
    ...
    % Nadia's changes: Make all the matrices symmetric
    for k = 0:L
        P_ = Vxf.P(:,:,k+1); P_t = Vxf.P(:,:,k+1)';
        ceq = [ceq; P_(:) - P_t(:)];
    end    
end
