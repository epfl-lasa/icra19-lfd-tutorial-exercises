...
for m = 1:length(models_to_eval)
    ...
    for j = 1:repetitions
        ...
        for i = 1:length(constr_type)
            ...
            if constr_type(i) == 0 || constr_type(i) == 1
                init_cvx    = 0; P_opt = [];
            else
...
if show_plot
    ...
    ylabel('RMSE','Interpreter','LaTex', 'FontSize',20)
    title('Reproduction Accuracy PC-GMM LPV-DS (Train) ', 'Interpreter','LaTex', 'FontSize',16)
    ...
    ylabel('RMSE','Interpreter','LaTex', 'FontSize',20)
    title('Reproduction Accuracy PC-GMM LPV-DS (Test) ', 'Interpreter','LaTex', 'FontSize',16)
