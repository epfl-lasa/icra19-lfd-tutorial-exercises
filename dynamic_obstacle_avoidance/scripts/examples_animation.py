...
def main(simulationNumber=0, saveFigures=False):
    ...
    if simulationNumber == 1:
        ...
        def func_w1(t):
            ...
            for ii in range(len(t_interval1)-1):
                if t < t_interval1[ii+1]*1*t_fact:
                    return (w1[ii+1]-w1[ii])/(t_interval1[ii+1]-t_interval1[ii]) * pi/180 * 1/t_fact
            return 0
        ...
        def func_w2(t):
            ...
            for ii in range(len(t_interval)-1):
                if t < t_interval[ii+1] * t_fact:
                    return (w[ii+1]-w[ii])/(t_interval[ii+1]-t_interval[ii]) * pi/180 * 1/t_fact
            return 0
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.002, N_simuMax=750, convergenceMargin=0.3, sleepPeriod=0.01, attractorPos=attractorPos, hide_ticks=False, saveFigure=saveFigures, animationName='replication_humans')
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.001, N_simuMax=1040, convergenceMargin=0.3, sleepPeriod=0.01)
    ...
    if simulationNumber==7:
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.01, N_simuMax=400, convergenceMargin=0.3, sleepPeriod=0.001, RK4_int=True, saveFigure=saveFigures)
    ...
    if simulationNumber==8:
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.01, N_simuMax=800, convergenceMargin=0.3, sleepPeriod=0.01, RK4_int=True, animationName="animation_ring_convergence")
    ...
    if simulationNumber ==9:
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.005, N_simuMax=620, convergenceMargin=0.3, sleepPeriod=0.01, saveFigure=saveFigures, animationName="rotatingEllipse")
    ...
    if simulationNumber ==10:
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.005, N_simuMax=620, convergenceMargin=0.3, sleepPeriod=0.01, saveFigure=saveFigures, animationName="rotatingEllipseWithStaticp")
    ...
    if simulationNumber ==11:
        ...
        run_animation(x_init, obs, x_range=x_range, y_range=y_range, dt=0.005, N_simuMax=600, convergenceMargin=0.3, sleepPeriod=0.01)
